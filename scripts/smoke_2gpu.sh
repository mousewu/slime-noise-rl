...
TASK_HF_CHECKPOINT="${HF_CHECKPOINT}"
TASK_ALFWORLD_ROOT="${ALFWORLD_ROOT}"
TASK_MEGATRON_CHECKPOINT="${MEGATRON_CHECKPOINT:-${TASK_HF_CHECKPOINT}_torch_dist}"
TASK_MANIFEST="${SMOKE_MANIFEST:-${TASK_PROJECT_DIR}/data/alfworld/train-smoke-8.jsonl}"
...
if [[ ! -d "${TASK_ALFWORLD_ROOT}" ]]; then
  echo "ALFWORLD_ROOT is not a local directory: ${TASK_ALFWORLD_ROOT}" >&2
  exit 2
fi
TASK_HF_CHECKPOINT="$(cd -- "${TASK_HF_CHECKPOINT}" && pwd)"
...
echo "[3/7] Validating the local Qwen3-4B Hugging Face checkpoint"
"${TASK_PYTHON_BIN}" -c \
  'import sys; from noise_rl.preflight import validate_local_hf_checkpoint; validate_local_hf_checkpoint(sys.argv[1])' \
  "${TASK_HF_CHECKPOINT}"
...
echo "[5/7] Preparing a manifest from the local ALFWorld games"
if [[ ! -d "${TASK_ALFWORLD_ROOT}/train" ]]; then
  echo "Missing local ALFWorld train split: ${TASK_ALFWORLD_ROOT}/train" >&2
  exit 2
fi
